...
@st.cache_data def load_labels(): categories = [] for r in category_table.all(): name = r.get("fields", {}).get("Category_Name_EN", "") if name: categories.append(name)
...
categories, subcategories = load_labels() category_tokens = clip.tokenize(categories).to(device) subcategory_tokens = clip.tokenize(subcategories).to(device)
...
def extract_first_image(file): if file.type == "application/pdf": return pdf2image.convert_from_bytes(file.read())[0] return Image.open(file)

def classify_content(image): text = pytesseract.image_to_string(image) image_input = preprocess(image).unsqueeze(0).to(device)
...
if uploaded_files: for file in uploaded_files: image = extract_first_image(file) st.image(image, caption=f"Preview: {file.name}", use_column_width=True)
